<template>
	<view class="container">
		<view class="header bg-purple-light">
			<view class="row-l-r">
				<view class="title-text">工菲记录</view>
				<view>
	...
	export default {
		data() {
			...
			return {
				...
				//minDate: Number(new Date()),//Number(new Date(this.addDate(timeFormat(new Date(), 'yyyy/mm/dd'), -730))),
				//maxDate: Number(new Date()),//Number(new Date(this.addDate(timeFormat(new Date(), 'yyyy/mm/dd'), 365))),
		...
		methods: {
			...
			async initDt(){
				...
				//this.minDate =Number(new Date(this.addDate(await timeFormat(new Date(), 'yyyy/mm/dd'), -730)));
				//this.maxDate = Number(new Date(this.addDate(await timeFormat(new Date(), 'yyyy/mm/dd'), 365)));
				//this.dt1 = Number(new Date());//Number(new Date(this.addDate(await timeFormat(new Date(), 'yyyy/mm/dd'), -30)));
				//this.dt2 = Number(new Date());
				this.textDt1 = await timeFormat(this.dt1, 'yyyy/mm/dd');
				this.textDt2 = await timeFormat(this.dt2, 'yyyy/mm/dd');
			...
			async confirm(e) {
				...
				let timeFormat = uni.$u.timeFormat
				if (this.current == 1) {
					this.dt1 = await  timeFormat(e.value, 'yyyy/mm/dd')
					this.textDt1 = await timeFormat(e.value, 'yyyy/mm/dd')
					this.changedDt=true;
				}
				if (this.current == 2) {
					this.dt2 = await  timeFormat(e.value, 'yyyy/mm/dd')
					this.textDt2 = await timeFormat(e.value, 'yyyy/mm/dd')
					this.changedDt=true;
				}
				console.log(this.textDt1,this.textDt2)

<template>
	<view class="container">
		<view class="row-l-r " style="background-color: #F8F8FF;padding-top: 6px;">
			<view><u-form>
				<u-form-item label="条码: ">
					<u--input :focus="focusState" @blur="focusState = false" confirm-type="done" v-model="barcodeText" border="none" placeholder="输入条码, 按回车提交"  @confirm="save(barcodeText)"></u--input>
				</u-form-item>
			</u-form></view>
			<view style="padding-top:6px;">
				<u-button size="small" type="primary" @click="doScan"> 扫码 </u-button>
			</view>
		</view>
		<view class="header">
				...
				<view class="page-box">
					<view class="item-row" :class="index%2==0?'bg-1':'bg-2'" v-for="(item, index) in dataLists" :key="index">
						<u-row>
							<u-col span="8">
								<view>
									<span class="text-no">{{index+1}}. </span>[{{item.TBarcode||''}}]  <span style="color:royalblue;">{{item.TExpCorp||''}}</span>
								</view>
							</u-col>
							<u-col span="3">
								<view>
							...
							<u-col span="1" @click="edit(4,item,index)">
		...
		<u-modal :show="showFilter" title="搜索条件" :closeOnClickOverlay="true" :show-cancel-button="true"
			...
			<view style="padding-left: 25rpx;padding-right: 25rpx;">
				<u-form ref="uForm">
					...
					<u-form-item :labelWidth="labelWidth" label="快递公司" borderBottom
						@click="showExpressCorp=true; hideKeyboard()">
						<u--input v-model="expressCorp" disabled disabledColor="#ffffff" placeholder="请选择裁床号" border="none">
						</u--input>
		...
		<u-picker
			:show="showExpressCorp"
			:columns="expressCorpLists"
			keyName="name"
			@change="changePicker"
			@cancel="cancelPicker"
...
<script>
	...
	export default {
		...
		methods: {
			...
			doScan() {
				...
				uni.scanCode({
					success: function(res) {
						that.save(res.result);
					}
				});
			},
			async save(barcodeText){
				let itemRecord={
					TBarcode:barcodeText,
					TIsDelete:0
				}
				let params = {
				...
				// 	'title':'正在处理操作...'
				// });
				let res = await saveExpress(params);
				//console.log(' res=> ',res.record.record)
				try{
					if(this.dtIdx==0 && res.record.code==0){
						this.dataLists.unshift(res.record.record);
						this.queryInfo.count += 1;
					}
					if(res.record.code==2){
						this.showMsg('已经扫码.')
					}
				}catch(e){
					console.log('error.')
			...
			selectPicker(e) {
				let item = e.value[0];
				if(this.showExpressCorp)
					this.expressCorp=item._id

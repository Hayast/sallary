<template>
	<view class="container">
		<view class="header bg-purple-light">
			<view class="row-l-r">
				<view class="title-text">物料管理</view>
					...
					<view>
						<u--form>
							...
							<u-form-item label="名称:" :label-width="labelWidth" borderBottom>
								<u--input v-model="itemRecord.TName" border="none" placeholder="输入物料名称"
									:customType="inputType"></u--input>
							</u-form-item>
							
							<u-form-item label="单价:" :label-width="labelWidth" borderBottom>
								<u--input v-model="itemRecord.TPrice" border="none" placeholder="输入物料单价" type="number"
									:customType="inputType"></u--input>
							</u-form-item>
							<u-form-item :labelWidth="labelWidth" label="类型" borderBottom
								@click="showType=true;showActType=0; hideKeyboard()">
								{{typeLists[0][itemRecord.TType].name || ''}}
								<u-icon slot="right" name="arrow-right"></u-icon>
							</u-form-item>
							<u-form-item :labelWidth="labelWidth" label="规格" borderBottom
								@click="showSize=true;showActType=0; hideKeyboard()">
								{{itemRecord.TSize || ''}}
								<u-icon slot="right" name="arrow-right"></u-icon>
							</u-form-item>
							<u-form-item :labelWidth="labelWidth" label="属性" borderBottom
								@click="showColor=true;showActType=0; hideKeyboard()">
								{{itemRecord.TColor || ''}}
								<u-icon slot="right" name="arrow-right"></u-icon>
							</u-form-item>
							<u-form-item :labelWidth="labelWidth" label="单位" borderBottom
								@click="showUnit=true;showActType=0; hideKeyboard()">
								{{itemRecord.TUnit || ''}}
								<u-icon slot="right" name="arrow-right"></u-icon>
							</u-form-item>
						</u--form>
	...
	export default {
		...
		methods: {
			...
			async save(actType) {
				...
				let res = await saveMaterial(params);
				uni.hideLoading();
				if (actType == 2)
